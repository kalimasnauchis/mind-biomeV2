(() => {
  ...
  const microCreatures = Array.from({ length: 58 }, (_, index) => ({
    seed: index * 9.73 + 1.5,
    anchor: structures[index % structures.length],
  ...
  function drawOcean() {
    ...
    gradient.addColorStop(0.58, "rgba(13, 29, 39, 0.78)");
  ...
  function drawCreatureShape(creature, x, y, size, alpha, observed) {
    ...
    const angle = Math.sin(state.time * 0.00032 + creature.seed) * 0.8;
    ctx.save();
    ctx.globalAlpha = clamp(alpha, 0, 0.78);
    ctx.translate(x, y);
    ctx.rotate(angle);

    ctx.fillStyle = colorWithAlpha(creature.color, 0.72);
    ctx.beginPath();
    ctx.ellipse(0, 0, size * 1.25, size * 0.48, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.globalAlpha *= 0.58 + observed * 0.22;
    ctx.strokeStyle = colorWithAlpha(palette.oldCream, 0.48);
    ctx.lineWidth = Math.max(0.45, size * 0.12);
    for (let i = -1; i <= 1; i += 1) {
      ctx.beginPath();
      ctx.moveTo(-size * 0.2, i * size * 0.18);
      ctx.quadraticCurveTo(size * 0.35, i * size * 0.56 + Math.sin(creature.seed + i) * size * 0.3, size * 1.8, i * size * 0.62);
      ctx.stroke();
    }

    if (observed > 0.32 && creature.temperament === 2) {
      ctx.globalAlpha *= 0.6;
      ctx.strokeStyle = colorWithAlpha(creature.color, 0.52);
      ctx.beginPath();
      ctx.ellipse(0, 0, size * (2.1 + observed), size * (0.9 + observed * 0.5), 0, 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.restore();
  }
